...
- $\alpha$ (chosen before the study) sets the Type I-error rate
...
- **Regression:** Statistical method modeling relationships between predictors and **continuous** outcomes
- **Logistic Regression:** For binary outcomes (yes/no); actually estimates continuous *odds* of outcome
...
# Case Study 1: ED Triage in LMCs
...
# Study Design: Smart Triage Model

## Question: Can We Build a Rapid Triage Tool for Acutely Ill Children in Low-Resource Settings?

::: {.columns}
::: {.column width="48%"}
### Methodology
- **Population:** Children under 19 years with acute illness at ED in Jinja, Uganda (90% under 5 years)
- **Study Type:** Prospective cohort study for model development
- **Sample:** 1,612 participants (80:20 train/test split)
- **Primary Outcome:** Hospital admission for >24 hours
:::
...
::: {.column width="48%"}
### Model Development
- **Candidate Predictors:** 63 variables based on PS2 working group guidelines
- **Variable Selection:** Bootstrapped stepwise regression
- **Final Model:** 9-variable logistic regression
- **Risk Stratification:**
  - Low risk threshold: 8% (91% sensitivity)
  - High risk threshold: 40% (92% specificity)
...
- Temp: 37 $^\circ C$
- MUAC: 130 mm
- O$_2$ Sat: 98% (Transformed: $70.103 \times \log_{10}(101.687 - 98) - 55.833 \approx 13.302$)
- Parent concern: Yes (1)
...
- Calculated Result:
  - Logit(p) = `r round(logit_p_mawji, digits=2)`
  - Probability of admission = `r round(prob_admission_mawji, digits=2)` (`r round(prob_admission_mawji*100, digits=2)`%)
...
**Our patient:** p = (`r round(prob_admission_mawji*100, digits=2)`% → **Priority** (between 8% and 40%)
...
patient_prob <- prob_admission_mawji  # Using the calculated probability from previous chunk

ggplot(risk_df, aes(x = x, y = 0)) +
  # Base line with rounded ends
  geom_segment(aes(x = 0, xend = 1, y = 0, yend = 0), 
               linewidth = 10, color = "#e0e0e1", 
               lineend = "butt") +
  ...
  annotate("text", x = patient_prob, y = 0.05, 
           label = paste0("P(admission) = ", round(prob_admission_mawji*100, digits=2), "%"), 
           color = "#2196f3", size = 4) +
...
# Case Study 2: Negative Trial - Asthma
...
ggplot(patient_data, aes(x = x, y = y, fill = outcome)) +
  ...
  geom_text(aes(label = description), vjust = 0.75, size = 3.2) +
...
- **"Positive" Trials (Tähtinen et al.):**
  ...
  - Effect size (ARR, NNT) & CIs show magnitude/precision
